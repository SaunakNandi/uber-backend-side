...
router.get('/location/other-party',authMiddleware.anyUser,rideController.getOtherPartyLocation)
router.post('/confirm',authMiddleware.authCaptain,
    body('rideId').isMongoId().withMessage('Invalid ride id'),
    rideController.confirmRide
)

// doing it as post but done it as get request in video
router.post('/start-ride',authMiddleware.authCaptain,
    body('rideId').isMongoId().withMessage('Invalid ride id'),
    body('otp').isString().isLength({ min:6,max:6 }).withMessage('Invalid OTP Length'),
